...
- `ASYNC_BIND_WINDOW_SECONDS`：死亡 hook 返回后，允许原生掉落物稍后创建的短时间窗口；窗口内的掉落仍必须暴露能解析到击杀者的 `PickupablePlayerUid`。
- `ASYNC_BIND_RADIUS`：已废弃的诊断值。严格来源绑定默认不再读取它，因为位置兜底可能与挖矿、砍树、采集或其他世界掉落重叠。
...
如果 Palworld 改动死亡掉落或地图物体生命周期，Mod 可能需要小范围更新 hook。当前绑定逻辑刻意保守，目的是避免拾取无关的世界掉落物。
